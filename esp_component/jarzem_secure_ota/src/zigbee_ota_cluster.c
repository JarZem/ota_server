...
static bool process_secure_protocol(const char *payload)
{
    ...
    if (strncmp(payload, "A1|", 3) == 0 || strncmp(payload, "P1|", 3) == 0 ||
        strncmp(payload, "R1|", 3) == 0) return true;
    return false;
}
...
static void zigbee_ota_diag_task(void *arg)
{
    ...
    (void)zigbee_ota_send_command_payload(DIAG_PONG);
...
static void zigbee_ota_len_test_task(void *arg)
{
    ...
    while (s_len_test_payload_len != 0) {
        ...
        test[len] = '\0';
        ++iteration;
        (void)zigbee_ota_send_command_payload(test);
        for (unsigned elapsed = 0;
             elapsed < DIAG_REPEAT_MS && s_len_test_payload_len != 0;
             elapsed += 100) {
            vTaskDelay(pdMS_TO_TICKS(100));
        }
    }
...
static bool zigbee_ota_process_payload(const char *payload, size_t payload_len)
{
    ...
    ESP_LOGI(TAG, "MQTT/ZIGBEE RX endpoint=%u state=%s bytes=%u payload=%.*s",
             ZIGBEE_OTA_ENDPOINT, ota_secure_session_state_name(), (unsigned)payload_len,
    ...
    if (strncmp(payload, DIAG_LEN_PREFIX, strlen(DIAG_LEN_PREFIX)) == 0) {
        ...
        if (end == payload + strlen(DIAG_LEN_PREFIX) || *end != '\0' ||
            requested < DIAG_LEN_MIN || requested > DIAG_LEN_MAX) return true;
...
static esp_err_t process_binary_downlink(const uint8_t *body, size_t len)
{
    ...
    if (state == OTA_SEC_STATE_WAIT_CHALLENGE) {
        ...
        char random_b64[16];
        char sig_b64[96];
        ESP_RETURN_ON_ERROR(base64url_encode(body, OTA_SECURE_RANDOM_LEN, random_b64, sizeof(random_b64)),
                            TAG, "A random encode failed");
        ESP_RETURN_ON_ERROR(base64url_encode(body + OTA_SECURE_RANDOM_LEN,
                                             DEVICE_CREDENTIAL_SIGNATURE_RAW_LEN,
                                             sig_b64, sizeof(sig_b64)),
                            TAG, "A signature encode failed");
    ...
    if (state == OTA_SEC_STATE_WAIT_PROVISIONING) {
        ...
        ESP_RETURN_ON_ERROR(base64url_encode(body, len, encrypted_b64, sizeof(encrypted_b64)),
                            TAG, "P encoding failed");
...
{
    if (!zigbee_ota_control_is_enabled() || s_hello_task_started) return;
...
esp_err_t zigbee_ota_cluster_add_attrs(esp_zb_attribute_list_t *cluster)
{
    ...
    esp_err_t err = esp_zb_cluster_add_manufacturer_attr(cluster, ZIGBEE_OTA_CLUSTER_ID,
