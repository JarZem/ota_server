...
class _Z2MBridgeSession:
    ...
    def _request(self, action: str, body: dict) -> dict:
        ...
        if response.get("status") != "ok":
            raise ValueError(
                f"zigbee2mqtt_converter_{action}_failed:{response.get('error') or 'unknown_error'}"
            )
        return response
    ...
    def wait_until(self, predicate, description: str) -> dict[str, str]:
...
def _deploy_via_zigbee2mqtt(base_topic: str, name: str, code: str, legacy_names: list[str]) -> dict:
    ...
    try:
        ...
        return {
            ...
            "removed": removed,
            "loaded_names": sorted(final),
            "previous_present": name in before,
        }
    finally:
        session.stop()
...
def handle_zigbee2mqtt_publish(handler) -> None:
    try:
        ...
        canonical = PUBLISH_DOMAIN + project.encode() + b"|" + digest.encode()
        ...
        legacy_names = [f"{project}.project.mjs", f"{project}.ota.mjs"]
        deploy = _deploy_via_zigbee2mqtt(base_topic, expected_name, code, legacy_names)
        file_sha = hashlib.sha256(data).hexdigest()

        print(
            f"Zigbee2MQTT converter deployed through MQTT API file={expected_name} "
            f"build={version} changed={int(deploy['changed'])} sha256={file_sha[:12]} "
            f"bytes={len(data)} removed={deploy['removed']}",
            flush=True,
        )
        print(
            f"Zigbee2MQTT bridge/converters verified file={expected_name} "
            f"build_marker={version} loaded=1",
            flush=True,
        )
        ...
        _send_json(
            ...
            201,
            {
                ...
                "transport": "zigbee2mqtt-mqtt-api",
                "files": {expected_name: file_sha},
