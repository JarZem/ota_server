...
class _Z2MBridgeSession:
    ...
    def start(self):
        ...
        if not self.connected.wait(MQTT_TIMEOUT_SECONDS):
            self.stop(); raise ValueError("zigbee2mqtt_mqtt_connect_timeout")
    ...
    def _request(self, action: str, body: dict):
        ...
        if result.rc != mqtt.MQTT_ERR_SUCCESS: raise ValueError(f"zigbee2mqtt_converter_{action}_publish_failed_rc_{result.rc}")
        if not self.response_event.wait(MQTT_TIMEOUT_SECONDS): raise ValueError(f"zigbee2mqtt_converter_{action}_response_timeout")
        with self.lock:
            response = dict(self.response_payload or {}); self.expected_transaction = None
        if response.get("status") != "ok": raise ValueError(f"zigbee2mqtt_converter_{action}_failed:{response.get('error') or 'unknown_error'}")
        return response

    def save(self, name, code): return self._request("save", {"name": name, "code": code})
    def remove(self, name): return self._request("remove", {"name": name})
    ...
    def wait_until(self, predicate, description):
        ...
        while True:
            ...
            if remaining <= 0: raise ValueError(f"zigbee2mqtt_converter_verify_timeout:{description}")
            self.converters_event.clear(); self.converters_event.wait(min(remaining, 2))


def _deploy_via_zigbee2mqtt(base_topic, name, code, legacy_names):
    session = _Z2MBridgeSession(base_topic=base_topic, service=_mqtt_service()); session.start()
    try:
        before = session.snapshot(); previous_code = before.get(name)
        session.save(name, code); current = session.wait_until(lambda c: c.get(name) == code, f"save:{name}")
        removed = []
        for legacy in legacy_names:
            if legacy in current:
                session.remove(legacy); current = session.wait_until(lambda c, n=legacy: n not in c, f"remove:{legacy}"); removed.append(legacy)
...
def handle_zigbee2mqtt_publish(handler) -> None:
    try:
        ...
        if len(raw) != length: raise ValueError("truncated_zigbee2mqtt_bundle")
        request = json.loads(raw.decode("utf-8")); project = str(request.get("project") or "")
        if not PROJECT_RE.fullmatch(project): raise ValueError("invalid_project_name")
        if int(request.get("schema") or 0) != 2: raise ValueError("unsupported_zigbee2mqtt_publish_schema")
        version = str(request.get("firmware_version") or "").strip(); files = request.get("files"); expected_name = f"{project}.mjs"
        if not version or not isinstance(files, dict) or set(files) != {expected_name}: raise ValueError("invalid_single_converter_file_set")
        ...
        options = _load_options(); base_topic = str(options.get("mqtt_base_topic") or "zigbee2mqtt")
        deploy = _deploy_via_zigbee2mqtt(base_topic, expected_name, code, [f"{project}.project.mjs", f"{project}.ota.mjs"])
        file_sha = hashlib.sha256(data).hexdigest()
        record_converter_publish(version=version, project=project, filename=expected_name, sha256=file_sha,
                                 publisher_device_id=publisher['device_id'], loaded=True)
